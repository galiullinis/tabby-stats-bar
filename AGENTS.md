...
- **Polling**: единый центральный планировщик (`startPollTimer`, интервал
  `POLL_INTERVAL_MS` = 5s из [config.ts](src/config.ts)). Опрашиваются **только бары
  активной верхней вкладки** (`isSshTabActive` — `contains`-проверка против
  `app.activeTab`, для сплитов покрывает все видимые панели). Фоновые вкладки не
  опрашиваются. Немедленный опрос на `activeTabChange`. Guard `fetchGuards`
  (WeakMap по session) не даёт параллельных запросов к одной сессии.
...
- **[macOS, Stage 2] Команда метрик — один проход `ps`** (`ps -A -o %cpu= -o %mem=`
  + один awk) вместо двух полных сканов таблицы процессов. Семантика значений не
  изменилась (сумма по процессам, lifetime-average — точность остаётся открытым
  пунктом).
...
- **macOS-метрики неточны (частично улучшено в Stage 2):** теперь один проход `ps`,
  но CPU% — всё ещё сумма lifetime-average по процессам (может быть >100%), сеть = 0.
  Для мгновенного CPU нужен `top -l 2`/`iostat` (отложено).
...
## 8. Следующие задачи
- **Stage 2 — ГОТОВО** (центральный планировщик, active-only, 5s, кэш map, macOS).
...
- **Отложено:** мгновенный CPU на macOS (`top -l 2`); вынести `POLL_INTERVAL_MS`
  в настраиваемый параметр UI; покрыть планировщик интеграционными тестами (jsdom).
