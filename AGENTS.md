...
- **Stage 3.4 (готово) — disk monitoring по mount point'ам:**
  - `serverStats.diskStyle` = `'single'` (корень `/`, %, **дефолт**) | `'mounts'`
    (по точкам монтирования). Команда disk-секции добавляется только при `'mounts'`
    (`buildDiskMountsFragment`), парсинг — `parseDiskMounts` → `DiskMount[]`
    (`mount/usedBytes/totalBytes/availableBytes/usagePercent`).
  - Сбор: машинно-парсимый `df -P -B1` (Linux, байты) / `df -P -k` (macOS, ×1024) —
    **не** `df -h`. Фильтр только локальных устройств (`Filesystem ~ /^\\/dev\\//`),
    что отсекает виртуальные (tmpfs/overlay/proc) и сетевые (nfs/cifs) ФС.
  - Компактный UI (только bottom-bar): `/` + 2–3 самых заполненных значимых маунта
    (`selectCompactMounts`, значимый = ≥1 GiB), тонкий разделитель между ними, полный
    список — в `title`-tooltip (`formatMountsTooltip`). Подкраска по занятости.
  - Floating-панель показывает корень `/` (как раньше) — per-mount это bottom-bar
    фича (как sparkline). Тесты: +10 → **68**.
  - Команда проверена на реальном macOS. Верификация: `npm test` ✓ (68),
    `npx webpack` ✓ (exit 0), `npx tsc --noEmit` ✓.
- **Stage 3.5 (готово) — I/O Wait как first-class метрика:**
  - iowait считается в **ядре** по дельте `/proc/stat` (поле `$6` iowait добавлено в
    `DeltaSample.cpuIowait`; `computeDeltaStats` отдаёт `iowait%`) — **без отдельной
    команды и без `sleep`** (в отличие от старого `iostat -c 1 1`).
  - Опциональная (тумблер `serverStats.showIoWait`, по умолчанию off). Показывается
    в bottom-bar (секция «IOW») и floating-панели (плитка «IOW»), подкраска по порогам
    (`ioColorFor`/`getIoColor`: <10 зел., <30 жёлт., иначе крас.).
  - Удалён пресет «I/O Wait» (теперь это встроенная метрика, не shell-пресет).
