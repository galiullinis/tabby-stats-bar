...
- **Метрики без `sleep`**: команда возвращает сырые счётчики (`/proc/stat`,
  ...
  значения, `ps`). Timeout адаптивный (`adaptiveTimeoutMs`).
...
**Открыто:**
- **Supply-chain (S1) — частично закрыто (Stage 3):** импорт пресета теперь требует
  подтверждения с показом команды; `fetch` с timeout. Остаётся: нет проверки
  целостности/подписи источника.
...
- **Roadmap (отложено / на будущее):**
  ...
  - Проверка целостности пресетов (подпись/хэш) или встроенный доверенный список.
